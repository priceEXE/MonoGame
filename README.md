...
在示例场景中添加了创建主摄像机的方法
ECS框架v1.5.3
修复了精灵渲染不在物体中心的bug，增添了一个MathTool类来管理自定义的插值函数Lerp和随机数生成函数
修复了组件在运行时Update中Institate物体会造成运行时错误的bug，现在在组件中也可以Institate
修改了SimpleAI，模板敌人在靠近玩家至一定距离后会自动停下
增加了GameManager以演示如何在运行时通过预制体动态地实例化实体
增加了Collider2D组件用于处理碰撞，现在只能生成圆形碰撞箱，且碰撞箱大小与渲染的图像大小一致
增加了反射调用碰撞响应函数的功能，在碰撞响应函数中也能正常创建销毁实体
ECS框架v1.5.4
修改了SimpleAI，现在敌人具有侦察范围，玩家进入侦察范围就被敌人发现，敌人会一直追踪玩家直到与玩家碰撞或者超出侦察范围，追踪玩家时敌人会变成红色
修复了碰撞检测时一方的事件函数无法触发的问题
修改了GameManager生成敌人的逻辑，现在场上会一直维持有五个敌人
修复了Start函数会一直调用消耗性能的问题
增加了isActive字段用于控制组件的禁用与否（禁用功能尚未实装）
更新了示例场景
